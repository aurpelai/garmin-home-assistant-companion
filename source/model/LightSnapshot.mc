...
//
// The combined HA /api/template call returns a JSON object with two keys:
//   { "areas":  { areaName: [entityId, ...], ... },
//     "states": { entityId: true|false, ... } }
// LightSnapshot splits that body, wrapping the areas section into an
// area-ordered structure (from which the flat "all lights" list derives) and
// parsing the states section into an entity_id -> Boolean map.
//
...
class LightSnapshot {
    ...
    // response: the two-key { "areas" => ..., "states" => ... } Dictionary.
    // Each section parses defensively; a missing or malformed body yields an
    // empty snapshot.
