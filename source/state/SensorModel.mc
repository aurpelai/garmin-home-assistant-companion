import Toybox.Lang;

class SensorModel {
    public var id as String;
    // Null when the sensor's state was non-numeric; never degraded to a zero.
    public var value as Float or Null;
    // Home Assistant's own formatting, never reparsed.
    public var displayValue as String;
    public var unit as String or Null;
    public var deviceClass as String;
    public var name as String;
    public var available as Boolean;
    public var areaId as String or Null;

    function initialize(id as String, value as Float or Null, displayValue as String,
                        unit as String or Null, deviceClass as String, name as String,
                        available as Boolean, areaId as String or Null) {
        self.id = id;
        self.value = value;
        self.displayValue = displayValue;
        self.unit = unit;
        self.deviceClass = deviceClass;
        self.name = name;
        self.available = available;
        self.areaId = areaId;
    }
}
